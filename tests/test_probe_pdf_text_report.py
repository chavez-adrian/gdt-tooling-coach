...
class ProbePdfTextReportTests(unittest.TestCase):
    ...
    def test_report_opens_existing_pdf_with_pypdf_and_records_page_count(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            fake_reader = type(
                "FakeReader", (), {"pages": [FakePage(None), FakePage(None)]}
            )()
    ...
    def test_report_counts_extracted_text_metrics_from_sampled_pages_only(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            fake_reader = type(
                "FakeReader",
                (),
                {"pages": [FakePage("alpha beta"), FakePage(None), FakePage("gamma")]},
            )()
        ...
        self.assertEqual(report[0]["extracted_char_count"], 15)
    ...
    def test_cli_uses_default_manifest_and_processed_output_paths(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            project_root = Path(temp_dir)
            ...
            manifest_path.write_text(
                json.dumps(
                    [
                        {
                            "source_title": "CLI Missing Source",
                            "expected_local_path": "data/raw/missing.pdf",
                        }
                    ]
                ),
            ...
            exit_code = probe_pdf_text.main(["--project-root", str(project_root)])
